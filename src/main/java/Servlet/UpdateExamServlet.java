...
public class UpdateExamServlet extends HttpServlet {
    ...
    public void doGet(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
        ...
        RequestDispatcher dispatcher = request.getRequestDispatcher("updateExam.jsp");
    ...
    public void doPost(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
        ...
        if (examType == null || examType.isEmpty() || examType.length() > 50) {
            ...
            response.sendRedirect("updateExam?examId=" + examId);
            return;
        }
        ...
        boolean isCreated = examService.updateExam(exam);

        if (isCreated) {
            session.setAttribute("status", "success");
            session.setAttribute("validation", "Exam updated successfully.");
        } else {
            session.setAttribute("status", "failed");
            session.setAttribute("validation", "Failed to update exam.");
        }
